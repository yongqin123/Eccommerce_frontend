...
export class CartComponent {
  ...
  minusQuantity(event: Event) {
    ...
    var minus = event.currentTarget as HTMLButtonElement;
    ...
    var size = (document.getElementById(id?.toString()  + "_"  + itemId.toString() + "_size") as HTMLElement).innerHTML;
    
    (document.getElementById(id?.toString()  + "_"  + itemId.toString() + "_size") as HTMLSelectElement).value = size;
    console.log(quantity);
    if (quantity > 1) {
      quantity--; // subtract first
  ...
  public getCartsEmail() {
    this.service.getAllCartsEmail(localStorage.getItem("email")??"").subscribe(( data : Cart[] ) => {
      ...
      this.service.getItem(cart.itemId).subscribe((item: Item) => {
        ...
        (document.getElementById(cart.id?.toString()  + "_"  + cart.itemId.toString() + "_size") as HTMLSelectElement).value = cart.size;
      });
    });
    })
  }
  ...
  public deleteCart(event: Event) {
    var deletee = event.currentTarget as HTMLButtonElement;
    console.log(deletee.id);
    this.service.deleteCart(parseInt(deletee.id)).subscribe();
    window.location.href = "/cart";
  }
